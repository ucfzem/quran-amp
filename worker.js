...
<body>
    ...
    <script>
        ...
        let _audioRetryIndex = 0;
        let _audioUrlChain = [];
        function playUrlChain(urls, idx) {
            if (idx >= urls.length) { showToast("تعذر تحميل الصوت", 3000); return; }
            _audioUrlChain = urls;
            _audioRetryIndex = idx + 1;
            audio.src = urls[idx];
            audio.load();
        }
        ...
        function setupEventListeners() {
            audio.addEventListener('error', () => {
                if (_audioRetryIndex > 0 && _audioRetryIndex < _audioUrlChain.length) {
                    playUrlChain(_audioUrlChain, _audioRetryIndex);
                }
            }, true);
